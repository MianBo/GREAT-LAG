...
namespace gnut {
	...
	// constructor
	// ----------
	t_rinexo_leo::t_rinexo_leo(t_gsetbase* s, string version, int sz)
		: t_rinexo3_leo(s, version, sz)
	{
	}
	...
	// ----------
	int t_rinexo_leo::decode_head(char* buff, int sz, vector<string>& errmsg)
	{
		_mutex.lock();
	...
	// ----------
	int t_rinexo_leo::decode_data(char* buff, int sz, int& cnt, vector<string>& errmsg)
	{
		...
		if (_rec.find(_site) == _rec.end() &&
			...
			if (_spdlog) SPDLOG_LOGGER_INFO(_spdlog,  "rinexo", _site + " marker name not found in configuration [skipped].");
	...
	// RINEX encoder
	// ---------
	int t_rinexo_leo::encode_head(char* buff, int sz, vector<string>& errmsg)
	{
		_mutex.lock();
				...
				if (it->second->id_type() == t_gdata::ALLOBJ) {
					...
					if (allGrec.begin() != allGrec.end()) {
						...
						if (hdrFromGrec.begin() != hdrFromGrec.end()) {
							...
							//write GLONASS SLOT / FRQ £¨if present£©
							auto glofrq = rnxHdr.glofrq();
							int nsat = glofrq.size();
							if (nsat != 0)
							{
								int ii = 0;
								_ss << setw(3) << nsat;
								for (auto it = glofrq.begin(); it != glofrq.end(); it++)
								{
									string sat = it->first;
									int freq = (int)it->second[0].second;
									_ss << setw(4) << sat << setw(3) << freq;
									if (++ii % 8 == 0) _ss << " GLONASS SLOT / FRQ #\n   ";
								}
								if (ii % 8 != 0)_ss << setw((8-ii%8) * 7) << " " << " GLONASS SLOT / FRQ #\n";
							}
	...
	// RINEX encode   
	// ---------
	int t_rinexo_leo::encode_data(char* buff, int sz, int& cnt, vector<string>& errmsg)
	{
		_mutex.lock();
	...
	int t_rinexo_leo::_decode_head()
	{
		while (_complete && ((_tmpsize = t_gcoder::_getline(_line)) >= 0)) {
			...
			if (_line.find("RINEX VERSION", 60) != string::npos) {          // first line
				...
				if (_spdlog) SPDLOG_LOGGER_INFO(_spdlog,  "rinexo", "END OF HEADER ");
				...
				else if (_version < "4.00") t_rinexo3_leo::_check_head();
				return(_consume = -1);
			}

			else if (_version < "3.00") t_rinexo2::_decode_head();
			else if (_version < "4.00") t_rinexo3_leo::_decode_head();
	...
	int t_rinexo_leo::_decode_data()
	{
		while (_complete && ((_tmpsize = t_gcoder::_getline(_line)) >= 0)) {
			...
			else if (_version < "4.00") t_rinexo3_leo::_decode_data();
